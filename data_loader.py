...
def _load_samples(csv_name, image_type):
    ...
    print 'filename_i_{}'.format(filename_i.shape)
...
def load_data(dataset_name, image_size_before_crop,
    ...
    image_i, image_j ,filename_i= _load_samples(
        csv_name, cyclegan_datasets.DATASET_TO_IMAGETYPE[dataset_name])
    inputs = {
        'image_i': image_i,
        'image_j': image_j
    }
    print 'inputs imagei{}'.format(image_i)
    # Preprocessing:
    inputs['image_i'] = tf.image.resize_images(
        inputs['image_i'], [64, 256 ])
    print 'inputs imageA{}'.format(inputs['image_i'])
    inputs['image_j'] = tf.image.resize_images(
      inputs['image_j'], [ 64, 256 ])
