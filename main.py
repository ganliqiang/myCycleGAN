...
class CycleGAN:
    ...
    def __init__(self, pool_size, lambda_a,
                 ...
                 dataset_name, checkpoint_dir, do_flipping, skip, filenames):
        ...
        self._samples_dir= os.path.join(self._output_dir, 'samples')
    ...
    def save_images(self, sess, epoch):
        ...
        #names = ['inputA_', 'inputB_', 'fakeA_',
         #        'fakeB_', 'cycA_', 'cycB_']
        testA =data.imagedata('/home/user/PycharmProjects/cycleGAN/cyclegan-1/input/horse2zebra/testA')
        testB =data.imagedata('/home/user/PycharmProjects/cycleGAN/cyclegan-1/input/horse2zebra/testB')
        with open(os.path.join(
            ...
            for i in range(0, self._num_imgs_to_save):
               ...
               # file_i = sess.run(self.filename_i)

                #print file_i
                real_a,A_name =testA(1)
                (filepath, tempfilename) = os.path.split(A_name)
                (shortname, extension) = os.path.splitext(tempfilename)
                print shortname

                #print type(A_name)
                #/home/user/PycharmProjects/cycleGAN/cyclegan - 1/input /horse2zebra/testA/3789
                #RK8NA347K4AC0086.png
                a_name=''
                ...
                tensors = [real_a, real_b,
                          fake_B_temp, fake_A_temp, cyc_A_temp, cyc_B_temp]
                for name, tensor in zip(names, tensors):
                    #sleep(0.5)
                    if (name == 'fakeA_'):
                        #name = A_name
                        image_name = shortname + str(epoch) + "_" + str(i) + ".bmp"
                        imsave(os.path.join(self._samples_dir, image_name),
                           ((tensor[0] + 1) * 127.5).astype(np.uint8))
                        v_html.write(
                            "<img src=\"" +
                            os.path.join('samples', image_name) + "\">"
                        )
                        #imsave(A_name,((tensor[0] + 1) * 127.5).astype(np.uint8))
                    else:
                        image_name = name + str(epoch) + "_" + str(i) + ".bmp"
                        imsave(os.path.join(self._images_dir, image_name),
                               ((tensor[0] + 1) * 127.5).astype(np.uint8))
                    #name = self.filenames[i]
                    #print (tensor[0]+1)*127.5
                    #image_name = name + str(epoch) + "_" + str(i) + ".bmp"
                    #imsave(os.path.join(self._images_dir, image_name),
                    #((tensor[0] + 1) * 127.5).astype(np.uint8))
    ...
    def train(self):
        ...
        #self.inputs['images_i']=tf.transpose(self.inputs['images_i'],perm=[0,2,1,3])
        #self.inputs['images_j'] = tf.transpose(self.inputs['images_j'], perm=[0, 2, 1, 3])
    ...
    def test(self):
        ...
        b=0
        self.inputs,self.filename_i = data_loader.load_data(
            self._dataset_name, self._size_before_crop,
            False, self._do_flipping)
        self.model_setup()
        saver = tf.train.Saver()
        init = tf.global_variables_initializer()
        print '410{}'.format(self.filename_i.shape)
        with tf.Session() as sess:
            sess.run(init)
...
def main(to_train, log_dir, config_filename, checkpoint_dir, skip):
    ...
    def get_filename(filename):
        ...
        return shotname
    filenames=[]
    with open('/home/user/PycharmProjects/cycleGAN/cyclegan-1/input/horse2zebra/horse2zebra_test.csv', 'rb') as csvfile:
        reader = csv.reader(csvfile)
        columns = [row[0] for row in reader]
        for clo in columns:
            filename = get_filename(clo)
            filenames.append(filename[0:16])
